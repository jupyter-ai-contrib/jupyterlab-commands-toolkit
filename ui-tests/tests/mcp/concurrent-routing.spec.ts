 ...
 * concurrent calls, a client would end up with both documents (or the wrong
 * one); we assert each client has exactly its own document.
 *
 * Runs in both second-client modes (same context / separate context). Runs only
 * in the `mcp` nox env.
 */
...
test.describe('mcp integration: concurrent routing', () => {
  for (const mode of SECOND_CLIENT_MODES) {
    test(`overlapping tool calls each run on their own client only (${mode})`, async ({
      page,
      browser,
      baseURL
    }) => {
      ...
      // notebook carries a kernelspec so opening it does not pop a dialog).
      await page.evaluate(
      ...
      // Fire both at the same time; each sleeps 5s server-side, guaranteeing
      // the two calls (different client ids) overlap before either resolves.
